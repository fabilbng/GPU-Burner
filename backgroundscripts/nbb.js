const nbbSteps = {
    ...
    getSummaryPage: 'getSummaryPage',
    startCheckout: 'startCheckout',
    encryptCheckout: 'encryptCheckout',
...
const addToCardFetchRequestNbb = async (url, pid, category) => {
    return await fetch(`${url}/action/add_product`, {
        "redirect": "manual",
        "headers": {
            "accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9",
            "accept-language": "en-US,en;q=0.9,de;q=0.8",
...
const addToCardNbb = (url, pid, category, tabId) => {
    chrome.tabs.get(tabId, async () => {
        if (chrome.runtime.lastError){
            ...
            try {
                const response = await addToCardFetchRequestNbb(url, pid, category)
                if (response.status === 0){
...
const fullRequestCheckoutNbb = (url, pid, category, options, klarnaSessionId, klarnaTokenId, pToken, pseudoCardPan, step, tabId) => {
    chrome.tabs.get(tabId, async () => {
        if (chrome.runtime.lastError){
            console.log('Closed Tab')
        }
            ...
            switch (step){
                //ADD TO CARD STEP
                case nbbSteps.addToCard: {
                    sendNotif(tabId,'Adding To Card...')
                    try {
                        const response = await addToCardFetchRequestNbb(url, pid, category)
                        if (response.status === 0){
                            sendNotif(tabId,'Successfully Added To Cart!')
                            fullRequestCheckoutNbb(url, pid, category, options, klarnaSessionId, klarnaTokenId, pToken, pseudoCardPan, nbbSteps.getCheckoutPage, tabId)
                        }
                        else {
                            throw 'Error adding to card'
                        }
                        ...
                        sendNotif(tabId,'Error Adding To Card Retrying...')
                        await timeout(1000)
                        fullRequestCheckoutNbb(url, pid, category,options, klarnaSessionId, klarnaTokenId, pToken, pseudoCardPan, nbbSteps.addToCard, tabId)
                    }
                    break
                }

                case nbbSteps.getCheckoutPage: {
                    sendNotif(tabId, 'Getting Checkout Page...')
                    try {
                        const response = await fetch('https://www.notebooksbilliger.de/kasse/unregistriert')
                        if (response.status === 200){
                            const htmlString = await response.text()
                            const htmlJQuery = $.parseHTML(htmlString)
                            const klarnaTokenId = $("#klarna_client_token", htmlJQuery).attr('value')
                            const klarnaSessionId = $("[name=klarna_session_id]", htmlJQuery).attr('value')
                            fullRequestCheckoutNbb(url, pid, category, options, klarnaSessionId, klarnaTokenId, pToken, pseudoCardPan, nbbSteps.startCheckout, tabId)
                        }
                        else {
                            throw 'Error'
                        }
                    } catch (error) {
                        sendNotif(tabId, 'Error Getting Checkout Page Retrying...')
                        await timeout(1000)
                        fullRequestCheckoutNbb(url, pid, category, options, klarnaSessionId, klarnaTokenId, pToken, pseudoCardPan, nbbSteps.getCheckoutPage, tabId)
                    }
                    break
                ...
                case nbbSteps.startCheckout: {
                    ...
                    try{
                        ...
                        if (response.status === 0){
                            ...
                            fullRequestCheckoutNbb(url, pid, category, options, klarnaSessionId, klarnaTokenId, pToken, pseudoCardPan, nbbSteps.getSummaryPage, tabId)
                        }
                        else {
                            throw 'Error'
                        }
                    } catch (error){
                        sendNotif(tabId, 'Error Starting Checkout. Retrying...' )
                        await timeout(1000)
                        fullRequestCheckoutNbb(url, pid, category, options, klarnaSessionId, klarnaTokenId, pToken, pseudoCardPan, nbbSteps.startCheckout, tabId)
                    }
                    break
                ...
                case nbbSteps.getSummaryPage: {
                    sendNotif(tabId, 'Getting Summary Page...')
                    try {
                        const response = await fetch("https://www.notebooksbilliger.de/kasse/zusammenfassung")
                        console.log(response)
                        if(response.status === 200){
                            sendNotif(tabId, 'Successfully got Summary Page')
                            const htmlString = await response.text()
                            const htmlJQuery = $.parseHTML(htmlString)
                            const pToken = $("#_ptoken", htmlJQuery).attr('value')
                            fullRequestCheckoutNbb(url, pid, category, options, klarnaSessionId, klarnaTokenId, pToken, pseudoCardPan, nbbSteps.encryptCheckout, tabId)
                        }
                        else {
                            throw 'Error getting Summary Page. Retrying...'
                        }
                    } catch(error){
                ...
                case nbbSteps.encryptCheckout: {
                    ...
                    try {
                        ...
                        if (response.status === 200){
                            ...
                            if (resultObject.status === 'VALID'){
                                sendNotif(tabId,'Success! Completing Checkout...')
                                const pseudoCardPan = resultObject.pseudocardpan
                                fullRequestCheckoutNbb(url, pid, category, options, klarnaSessionId, klarnaTokenId, pToken, pseudoCardPan, nbbSteps.completeCheckout, tabId)
                            }
                            else if(resultObject.status === 'INVALID') {
                                sendNotif(tabId,'Invalid Card Details')
                            }
                            else throw 'Unknown CC-Error (banned?)'
                        }
                        else {
                            throw 'Error Encrypting Checkout. Retrying...'
                        }
                    } catch (error){
                        sendNotif(tabId, error)
                        await timeout(1000)
                ...
                case nbbSteps.completeCheckout: {
                    sendNotif(tabId, 'Completing Checkout...')
                    try {
                        const response = await fetch("https://www.notebooksbilliger.de/checkout_process.php", {
                            "headers": {
                        ...
                        if (response.status === 200){
                            ...
                            throw 'Error Completing Checkout. Retrying...'
                        }
                    } catch (error) {
                        sendNotif(tabId, error)
                        console.log(error)
...
chrome.runtime.onMessage.addListener(
    (request, sender) => {
        ...
        if(request.message === 'checkoutPossibleNbb'){
            if (isValid){
                if(options.nbbNormalEnabled){
                    addToCardNbb(request.url, request.productId, request.categoryId, sender.tab.id)
                }
                else if (options.nbbRequestEnabled){
                    fullRequestCheckoutNbb(request.url, request.productId, request.categoryId, options, null, null, null, null,  nbbSteps.addToCard, sender.tab.id)
                }
                else {
